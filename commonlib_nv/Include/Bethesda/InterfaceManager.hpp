...
public:
	...
	struct VATSHighlightData {
		...
		void HandleOcclusionQueries(BSRenderedTexture* apCurrentNodeTexture, QueryType aeQueryType, bool a4, int a5, NiRenderer::ClearFlags uiClearMode, bool abEndFrame) {
			ThisStdCall(0x800F30, this, apCurrentNodeTexture, aeQueryType, a4, a5, uiClearMode, abEndFrame);
		}
	...
	UInt32							unk024;	
	TileImage*						pCursor;
	NiPoint3						kCursorDistFromCenter;
	NiPoint3						kCursorPos;
	...
	static bool IsMenuActive(UInt32 auiMenuType);

	static bool	 IsMenuVisible(UInt32 menuType) {
		return CdeclCall<bool>(menuType, 0);
	}

	static Tile* GetMenuByType(UInt32 menuType) {
		return CdeclCall<Tile*>(0xA09030, menuType);
	}
	...
	void Render(int unused0, int unused1);
};
