...
public:
	...
	NiTList<TESObjectREFR> mapMarkerList; // 0D8
	void* doorList; // 0E0
	Float32 flt0E8; // 0E8
	Float32 flt0EC; // 0EC
	Float32 flt0F0; // 0F0
	...
	void SetupLinesToMarker(TESObjectREFR* lastExtDoor, TESObjectREFR* ref, TeleportPath* questTarget, signed int tileID);

	bool ScrollNotes(MenuSpecialKeyboardInputCode direction, bool isShiftHeld);
};

ASSERT_SIZE(MapMenu, 0x230);
ASSERT_OFFSET(MapMenu, numLoadeduGrids, 0x128)