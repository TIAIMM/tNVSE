...
struct BSTextureManager {
	...
	NiDepthStencilBufferPtr					spShadowDepthBuffer;
	NiObjectPtr								spUnk0AC;

	static __forceinline BSTextureManager* GetSingleton() { return *(BSTextureManager**)0x11F91A8; }

	BSRenderedTexture* BorrowRenderedTexture(NiDX9Renderer* apRenderer, UInt32 aeType, NiRefObjectPtr spTexture1, NiRefObjectPtr spTexture2, NiRefObjectPtr spTexture3);
	// Separate func to avoid NiPointer refcounting
	BSRenderedTexture* BorrowRenderedTexture(NiDX9Renderer* apRenderer, UInt32 aeType);
	BSRenderedTexture* BorrowRenderedTexture(NiDX9Renderer* apRenderer, UInt32 aeType, UInt32 unk, UInt32 uiBackgroundColor);

	// Looks like it cleans up textures from lists
	__declspec(noinline) void ReturnRenderedTexture(BSRenderedTexture* apRenderedTexture);
	void ReturnFrustumShadowTexture(BSRenderedTexture* apShadowTexture);
	void Update(NiCamera* apCamera = nullptr, bool abIsMenuMode = false);

	void SetFrustumShadowTextureCount(NiRenderer* apRenderer, UInt32 auiActorShadowCount);
	BSRenderedTexture* BorrowFrustumShadowTexture();
	...
	static BSRenderedTexture* CreateRenderedTexture(UInt32 uiWidth, UInt32 uiHeight, UInt32 auiFlags, D3DFORMAT eFormat, UInt32 auiMSAA) {
		...
		return ThisStdCall<BSRenderedTexture*>(0xB6D170, GetSingleton(), NiDX9Renderer::GetSingleton(), uiWidth, uiHeight, eFormat, 0, auiFlags, 0, auiMSAA, 0, 0);
	}

	static BSRenderedTexture* CreateRenderedCubemap(UInt32 uiSize, UInt32 auiFlags, D3DFORMAT eFormat, UInt32 auiMSAA);

	void LookupTextureParameters(NiDX9Renderer* apRenderer, RenderTargetTypes aeRenderTargetType, UInt32* apWidth, UInt32* apHeight, D3DFORMAT* apFormat, DWORD* a6, UInt32* apFlags, D3DMULTISAMPLE_TYPE* apMSAA);
