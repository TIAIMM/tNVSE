...
public:
	...
	void ProcessEx(NiAVObject* apObject) { 
		BSCullingProcess::Process(apObject); 
	}
	void ProcessAltEx(const NiCamera* apCamera, NiAVObject* apScene, NiVisibleArray* apVisibleSet) {
		BSCullingProcess::ProcessAlt(apCamera, apScene, apVisibleSet);
	}
	void AppendVirtualEx(NiGeometry* apGeom) {
		BSCullingProcess::AppendVirtual(apGeom);
	}
};
