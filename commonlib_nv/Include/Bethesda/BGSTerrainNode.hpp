#pragma once

#include "NiPoint3.hpp"
#include "NiPoint2.hpp"
#include "NiSmartPointer.hpp"
#include "BGSDistantTreeBlock.hpp"

class BGSTerrainChunk;
class BGSDistantObjectBlock;

class BGSTerrainManager;

struct BGSTerrainNode {
	union Coordinate {
		UInt32 xy;
		struct {
			SInt16 x;
			SInt16 y;
		};
	};

	BGSTerrainManager*		pManager;
	UInt32					uiLODLevel;
	Coordinate				sBaseCellCoords;
	bool					bIsLvlMaxOrLower;
	UInt8					byte0D;
	UInt8					byte0E;
	UInt8					byte0F;
	BGSTerrainChunk*		pTerrainChunk;
	BGSDistantObjectBlock*	pObjectBlock;
	BGSDistantTreeBlock*	pTreeBlock;
	UInt32					ukn1C;
	BGSTerrainNode*			pChildren[4];
	BGSTerrainNode*			pParent;
	NiPoint3				pos34;
	float					fDist;
	float					fSplitDist;
	float					fMorphStartDist;
	float					fMorphEndDist;
	UInt32					uiTerrainFadeTimer;
	bool					bForceFade;
	UInt32					uiLinkID;
	bool					maybeShouldShow_5C;
	bool					byte5D;
	bool					byte5E;
	bool					byte5F;

	SInt16 GetCellX() const;
	SInt16 GetCellY() const;
	BGSTerrainChunk* GetTerrainChunk() const;
	BGSDistantObjectBlock* GetObjectBlock() const;
	BGSDistantTreeBlock* GetTreeBlock() const;
	UInt32 GetLODLevel() const;
	float GetRange(const NiPoint2 apPos1, const NiPoint2 apPos2, float afScale);
	bool IsWithinLoadDistance(const NiPoint3& apPosition);
	bool IsWithinLowLoadDistance(const NiPoint3& apPosition);
	bool IsPlayerInRange() const;
	void RenderDistantBlocksRecurse(NiPoint3& apPlayerPos, bool abIsBlocks);
	void UpdateBlockVisibility(bool abShow = false);

	BGSDistantTreeBlock* GetDistantTreeBlock(NiPoint3* apPos);
};

ASSERT_SIZE(BGSTerrainNode, 0x60);