...
class __declspec(align(8)) Archive : public BSFile, public NiRefObject , public BSArchive {
public:
	...
	ASSERT_SIZE(BSCriticalSection, 0x28);

	__int64 ArchiveFileTime;
	unsigned int iFileNameArrayOffset;
	unsigned int iLastDirectoryIndex;
	unsigned int iLastFileIndex;
	BSCriticalSection ArchiveCriticalSection;
	unsigned __int8 cArchiveFlags;
	char* pDirectoryStringArray;
	unsigned int* pDirectoryStringOffsets;
	char* pFileNameStringArray;
	unsigned int** pFileNameStringOffsets;
	int m_iRuntimeID;
	...
	bool FindFile(const BSHash& arDirectoryHash, const BSHash& arFileNameHash, UInt32& arDirectoryID, UInt32& arFileID, const char* apFileName);
	bool FindDirectory(const BSHash& arDirectoryHash, UInt32& aruiDirectoryIndex, const char* apFileName);
	bool FindFileInDirectory(UInt32 auiDirectoryIndex, const BSHash& arFileNameHash, UInt32& aruiFileIndex, const char* apFileName, bool abSkipSecondaryArchiveCheck);
...
public:
	...
	static ArchiveList* GetArchiveList() { return *(ArchiveList**)0x11F8160; }
	static void SetArchiveList(ArchiveList* apArchiveList) { *(ArchiveList**)0x11F8160 = apArchiveList; }
