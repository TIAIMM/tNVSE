...
#define EXTRADATATYPE(x) static constexpr UInt8 kExtraDataType = ED_##x;

class TESForm;

class BSExtraData {
public:
	BSExtraData(uint8_t code) {}
	virtual ~BSExtraData() = default;

	virtual bool Differs(BSExtraData* apExtra) {
		return ThisStdCall<bool>(0x40F700, this, apExtra);
	}
...
public:
	...
	__forceinline T_Data* GetExtraData() const {
		return (T_Data*)GetExtraData(T_Data::kExtraDataType);
	}
