...
public:
	...
	virtual void							SetAlphaBlendEnable(DWORD aValue, UInt32 unk0 = 0, UInt32 unk1 = 0);
	virtual void							SetDepthBias(float afBias, bool abSave = false);
	// Do not use
	virtual void							SetRenderState(D3DRENDERSTATETYPE eState, UInt32 uiValue, int, bool bSave);
	...
	// Do not use
	virtual void							vSetDeclaration(LPDIRECT3DVERTEXDECLARATION9 hDeclaration, bool bSave);
	...
	// Do Not Use
	virtual void							SetTexture(UInt32 uiStage, LPDIRECT3DBASETEXTURE9 pkTexture); 
	// Do Not Use
	virtual LPDIRECT3DBASETEXTURE9			GetTexture(UInt32 uiStage);
	...
	static UInt16 GetTextureStagesStatesRevMappings(UInt32 auiState);

	void					SetPixelShaderEx(LPDIRECT3DPIXELSHADER9 apShader, bool abSave = false);
	void					SetVertexShaderEx(LPDIRECT3DVERTEXSHADER9 apShader, bool abSave = false);
	void					SetModelTransform(const NiTransform& kXform);
	void					SetTextureEx(UInt32 uiStage, LPDIRECT3DBASETEXTURE9 pkTexture);
	LPDIRECT3DBASETEXTURE9	GetTextureEx(UInt32 uiStage) const;
	void					SetRenderStateEx(D3DRENDERSTATETYPE aeState, UInt32 auiValue, UInt32, bool abSave = false);
	void					SetTextureStageStateEx(UInt32 auiStage, D3DTEXTURESTAGESTATETYPE aeState, UInt32 auiValue, bool abSave = false);
};
